...
(function () {
  ...
  function update(dt) {
    ...
    const D = e + 100 * Math.pow(over / 1000, 1.5);
    ...
    // 스폰: 졸병 — 생성률(초당)이 D=0의 6마리 → D=180(3분)에 50마리로 상한.
    //   ease-in(제곱) 곡선이라 초반은 완만, 3분 직전에 급증.
    //   50 도달 후에는 "더 늘어나려던 생성률(초과분)의 절반"을 졸병 체력에 가산.
    const rateRaw = 6 + 44 * Math.pow(D / 180, 2); // 상한 없는 원곡선
    const gruntRate = Math.min(50, rateRaw);
    const overflow = rateRaw - gruntRate; // 50 초과분(늘어나려던 수)
