...
// 전투력 = 병력 × 등급
function power() {
  return Math.floor(run.army) * run.tier;
}
// 게이트 한쪽 적용: 수량형은 병력에, 등급형('t')은 등급에
function applySide(s) {
  if (s.op === "t") run.tier = Math.max(1, run.tier + s.val);
  else run.army = applyOp(run.army, s.op, s.val);
}

function resolveEvent(e) {
  e.resolved = true;
  const prevArmy = run.army;
  const prevPow = power();
  if (e.type === "gate") {
    // 부대 중심이 속한 한쪽 게이트 적용
    const side = run.px < CENTER ? e.left : e.right;
    applySide(side);
    const good = power() >= prevPow;
    spawnFlash(run.px, SQUAD_Y, good ? "#5cff8f" : "#ff4d6d", 10);
    if (run.army < prevArmy) triggerDamage(prevArmy - run.army);
  } else if (e.type === "enemy") {
    if (power() >= e.count) {
      // 돌파: 등급이 높을수록 피해(전사자) 감소
      const cas = Math.min(run.army, Math.ceil(e.count / (run.tier * 3)));
      run.army -= cas;
      run.flash = 0.14;
      spawnFlash(CENTER, SQUAD_Y - 30, "#ffd54d", 16);
      if (cas > 0) triggerDamage(cas);
    } else {
    ...
    if (power() >= e.count) {
      ...
      // 무한모드: 보스 돌파 후 계속(피해만 반영)
      const cas = Math.min(run.army - 1, Math.ceil(e.count / (run.tier * 3)));
      if (cas > 0) {
        run.army -= cas;
        triggerDamage(cas);
      }
...
function finish(result) {
  ...
  if (result === "win") {
    ...
    sub.textContent =
      "전투력 " + formatNum(power()) + " (🪖" + formatNum(run.army) + " ★" + run.tier + ")";
...
function update(dt) {
  ...
  hudArmy.textContent = "🪖 " + formatNum(run.army) + "  ★" + run.tier;
  if (hudPower) hudPower.textContent = "⚔ " + formatNum(power());
}
...
function opLabel(g) {
  if (g.op === "t") return g.val >= 0 ? "★+" + g.val : "★" + g.val;
  const sym = g.op === "*" ? "×" : g.op === "/" ? "÷" : g.op === "-" ? "−" : "+";
  return sym + g.val;
}
function sideGood(g) {
  return (g.op === "t" && g.val > 0) || g.op === "+" || g.op === "*";
}
...
function drawGateHalf(x1, x2, y, h, g) {
  const good = sideGood(g);
  const isTier = g.op === "t";
  // 등급 게이트는 보라/금색 계열, 수량 게이트는 초록/빨강
  const fill = isTier
    ? good
      ? "#4a338f"
      : "#5a2a55"
...
function drawEnemyGroup(e, y) {
  ...
  const shown = Math.min(12, Math.max(4, Math.round(Math.sqrt(e.count))));
...
const TIER_BODY = ["#4dd0e1", "#5cff8f", "#ffd54d", "#ff9f40", "#ff5bd0", "#b388ff"];
function drawSquad() {
  const n = Math.floor(run.army);
  const shown = Math.min(40, Math.max(1, n));
  const cols = 7;
  const pal = { head: "#ffd9a0", body: TIER_BODY[Math.min(TIER_BODY.length - 1, run.tier - 1)], leg: "#2a2150" };
  for (let i = 0; i < shown; i++) {
  ...
  ctx.fillText("★" + run.tier, run.px, SQUAD_Y - 46);
}
